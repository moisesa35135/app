...
# Ruta para manejar la solicitud PATCH para actualizar campos específicos de un anime por su ID
@app.route('/anime/<int:anime_id>', methods=['PATCH'])
def actualizar_campos_anime(anime_id):
    datos = request.json
    anime = next((anime for anime in base_de_datos if anime['id'] == anime_id), None)
    ...
    if anime:
        ...
        return jsonify({"mensaje": "Campos del anime actualizados exitosamente", "anime": anime}), 200
    else:
        return jsonify({"mensaje": "Anime no encontrado"}), 404
